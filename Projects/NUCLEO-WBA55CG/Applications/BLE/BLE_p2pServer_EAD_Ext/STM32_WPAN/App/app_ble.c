...
{
  8, AD_TYPE_COMPLETE_LOCAL_NAME, 'E', 'A', 'D', 'S', '_', 'X', 'X',  /* Complete name */
...
/* USER CODE BEGIN PV */
uint8_t encryption_key[16] = {0x57,0xA9,0xDA,0x12,0xD1,0x2E,0x6E,0x13,0x1E,0x20,0x61,0x2A,0xD1,0x0A,0x6A,0x19};
uint8_t encryption_key_little[16];
uint8_t initialization_vector[8] = {0x46,0xE7,0x7A,0xB1,0xEF,0x00,0x7A,0x9E};
uint8_t initialization_vector_little[8];
uint8_t char_encr_data_key_mat[24];
//uint8_t ead_AdvData[7] = {0x06, AD_TYPE_ENCRYPTED_ADV_DATA, 0x62,0x6F,0x6E,0x6A,0x6F};
uint8_t ead_AdvData[171];

uint16_t ead_data_encrypted_length;
uint8_t ead_data_encrypted[180];

uint16_t ead_data_encrypted_length_NCS;
uint8_t ead_AdvData_no_conn_no_scan[241];
uint8_t ead_data_no_conn_no_scan_encrypted[250];
extern uint8_t ead_data_encryp[7];
/* USER CODE END PV */
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  /* Add a new characterisitc "Encrypted Data Key Material" */
  Char_UUID_t  uuid;
...
//  APP_DBG_MSG("==>> End Ble_Hci_Gap_Gatt_Init function\n\r");
  ...
  /* set in little indian the key */
  for (i=0;i<sizeof(encryption_key);i++)
  {
    encryption_key_little[15-i] = encryption_key[i];
  }
  /* set in little indian the IV */
  for (i=0;i<sizeof(initialization_vector);i++)
  {
    initialization_vector_little[7-i] = initialization_vector[i];
  }
  
  for (i=0;i<sizeof(initialization_vector);i++)
  {
    char_encr_data_key_mat[i] = initialization_vector_little[i];
  }
  
  for (i=0;i<sizeof(encryption_key);i++)
  {
    char_encr_data_key_mat[i+sizeof(initialization_vector)] = encryption_key_little[i];
  }
  
  /* write key/vector in characteristic -fixed value for the moment */  
  ...
  ret = aci_hal_ead_encrypt_decrypt(0x00, encryption_key_little, initialization_vector_little, sizeof(ead_AdvData), &ead_AdvData[0], &ead_data_encrypted_length, ead_data_encrypted);

  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_hal_ead_encrypt_decrypt, result: 0x%02X\n", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_hal_ead_encrypt_decrypt\n");
  } 
  APP_DBG_MSG("  ead_data_encrypted_length: 0x%02X\n", ead_data_encrypted_length);
  
  /* Initialization advertising data No Conn-No Scan */
  for (i=0;i<241;i++)
  {
    ead_AdvData_no_conn_no_scan[i] = i;
  }
  /* call encryption function */
  ret = aci_hal_ead_encrypt_decrypt(0x00, encryption_key_little, initialization_vector_little, sizeof(ead_AdvData_no_conn_no_scan), &ead_AdvData_no_conn_no_scan[0], &ead_data_encrypted_length_NCS, ead_data_no_conn_no_scan_encrypted);
  ...
  APP_DBG_MSG("  ead_data_encrypted_length_NCS: 0x%02X\n", ead_data_encrypted_length_NCS);
}
...
{
  LOG_INFO_APP("restart adv no conn, no scan\n");
  /* start advertising: No connectable, no scannable */
  restart_adv_noconn_noscan();  
}

void APPE_Button2Action(void)
{
  LOG_INFO_APP("stop adv no conn, no scan\n");
  ADV_EXT_Stop(1);
}
