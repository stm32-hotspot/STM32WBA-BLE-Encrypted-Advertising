...
typedef struct
{
  ...
  uint8_t EAD_found;
  uint8_t EAD_length;
  uint8_t EAD_data[(BLE_EVT_MAX_PARAM_LEN - 1) - 25];
  uint8_t EAD_data_NCS[1650];
  /* USER CODE END PTD_1 */
...
/* USER CODE BEGIN GV */
const uint8_t *ead_data_decrypted;
const uint8_t *ead_data_received;
/* USER CODE END GV */
...
/* USER CODE BEGIN EV */
uint8_t Out_Data[228];
uint16_t Out_Data_Length;
extern uint8_t encryption_key[16];
extern uint8_t initialization_vector[8];
/* USER CODE END EV */
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          /* Discover services */
          result = aci_gap_is_device_bonded(p_enhanced_conn_complete->Peer_Address_Type, p_enhanced_conn_complete->Peer_Address);
          if (result == BLE_STATUS_SUCCESS)
          {
            APP_DBG_MSG("aci_gap_is_device_bonded success \r\n\r");
          }
          else
          {
            APP_DBG_MSG("aci_gap_is_device_bonded Failed \r\n\r");
          }
          
          if (result == BLE_STATUS_SUCCESS)
          {
            APP_DBG_MSG("device already bonded \n\r");
          }
          else
          {
      ...
      switch (p_blecore_evt->ecode)
      {
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE:
        {
          ...
          else
          {
            ...
            /* read char KeyMaterial */
            GATT_CLIENT_APP_Procedure_Gatt(0, PROC_GATT_READ_CHAR_KEY_MATERIAL);
          }
          LOG_INFO_APP("\n");
...
//      else
//      {
//        //UTIL_SEQ_WaitEvt(1U << CFG_IDLEEVT_MTU_COMPLETE);
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_APP("  Fail   : aci_gap_clear_security_db command, result: 0x%02X\n", ret);
    }
    else
    {
      LOG_INFO_APP("  Success: aci_gap_clear_security_db command\n");
    }
  ...
  else
  {
        ...
        (HAL_IS_BIT_SET(p_ext_adv_report->Event_Type, 0x0001)) )   /* connectable */
    {
      ...
      while ( i < p_ext_adv_report->Data_Length )
      {
        ...
        /* add encrypted advertising data */
        if ( adtype == AD_TYPE_ENCRYPTED_ADV_DATA)
        {
          uint16_t j;
          bleAppContext.EAD_found = 1;
          bleAppContext.EAD_length = adlength;
          for (j=0;j<bleAppContext.EAD_length - 1;j++)
          {
            bleAppContext.EAD_data[j] = p_ext_adv_report->Data[j+i+2];
          }
          /* call decryption function */
          ret = aci_hal_ead_encrypt_decrypt(0x01, encryption_key, initialization_vector, adlength - 1, bleAppContext.EAD_data, &Out_Data_Length, Out_Data);
          if (ret != BLE_STATUS_SUCCESS)
          {
        ...
        if ( adtype == AD_TYPE_MANUFACTURER_SPECIFIC_DATA )
        {
          if ( (adlength >= 7) && 
               ...
               (p_ext_adv_report->Data[i + 6] == FW_ID_P2P_EADS))   /* p2pServer application */
          {
    ...
    ////////////////
//    if( !(HAL_IS_BIT_SET(p_ext_adv_report->Event_Type, 0x0010)) && /* extended */
//      (bleAppContext.sid == p_ext_adv_report->Advertising_SID))
//    {
//      uint8_t i, adtype, adlength;
//      tBleStatus ret;
//      
//      i = 0;
//      
//      LOG_INFO_APP("  Advertising_SID %d\n", p_ext_adv_report->Advertising_SID);
//      while ( i < p_ext_adv_report->Data_Length )
//      {
//        adlength = p_ext_adv_report->Data[i];
//        adtype = p_ext_adv_report->Data[i + 1];
//        /* add encrypted advertising data */
//          uint16_t j;
//          LOG_INFO_APP("  encrypted data received \n");
//          bleAppContext.EAD_length = adlength;
//          for (j=0;j<bleAppContext.EAD_length - 1;j++)
//          {
//            bleAppContext.EAD_data_NCS[j] = p_ext_adv_report->Data[j+i+2];
//          }
//          /* call decryption function */
//          ret = aci_hal_ead_encrypt_decrypt(0x01, encryption_key, initialization_vector, adlength - 1, bleAppContext.EAD_data_NCS, &Out_Data_Length, Out_Data);
//          if (ret != BLE_STATUS_SUCCESS)
//          {
//            APP_DBG_MSG("  Fail   : aci_hal_ead_encrypt_decrypt, result: 0x%02X\n", ret);
//          }
...
//    }
    ...
    if( !(HAL_IS_BIT_SET(p_ext_adv_report->Event_Type, 0x0010)) && /* extended */
        (HAL_IS_BIT_SET(p_ext_adv_report->Event_Type, 0x0020)) )   /* no connectable, no scannable */
    {
      uint8_t i, adtype, adlength;
      
      i = 0;
      LOG_INFO_APP("  sid NCS %d\n", p_ext_adv_report->Advertising_SID);
      while ( i < p_ext_adv_report->Data_Length )
      {
        adlength = p_ext_adv_report->Data[i];
        adtype = p_ext_adv_report->Data[i + 1];
...
void APPE_Button2Action(void)
{
  ...
  {  
    /* send disconnection */
    result = aci_gap_terminate( bleAppContext.BleApplicationContext_legacy.connectionHandle, 0x13);
    if (result == BLE_STATUS_SUCCESS)
    {
    ...
    {
      APP_DBG_MSG("aci_gap_terminate fail, status =0x%02X\n\n", result);
    }
  }
}
