...
typedef struct
{
  ...
  /* handles of EAD Key Material characteristic */
  uint16_t EAD_Key_MaterialCharHdl;
  uint16_t EAD_Key_MaterialValueHdl;
  uint8_t EAD_Key_MaterialCharProperties;
...
uint8_t initialization_vector[8];
...
uint8_t GATT_CLIENT_APP_Procedure_Gatt(uint8_t index, ProcGattId_t GattProcId)
{
  ...
  else
  {
    ...
    switch (GattProcId)
    {
      ...
      case PROC_GATT_READ_CHAR_KEY_MATERIAL:
      {
        if (a_ClientContext[index].EAD_Key_MaterialCharHdl != 0x0000)
        {
          result = aci_gatt_read_char_value(a_ClientContext[index].connHdl,
                                            a_ClientContext[index].EAD_Key_MaterialCharHdl+1);
          if (result == BLE_STATUS_SUCCESS)
          {
            gatt_cmd_resp_wait();
            LOG_INFO_APP("aci_gatt_read_char_value success\n\n");
          }
          else
          {
            LOG_INFO_APP("aci_gatt_read_char_value fail, status =0x%02X\n\n", result);
          }
        }
      }
    default:
...
//          GATT_CLIENT_APP_Procedure_Gatt(0, PROC_GATT_READ_CHAR_KEY_MATERIAL);
        ...
        {
          aci_att_read_blob_resp_event_rp0 *read_blob_resp_event;
          read_blob_resp_event = (aci_att_read_blob_resp_event_rp0 *)p_blecore_evt->data;
          LOG_INFO_APP("  ACI_ATT_READ_BLOB_RESP_VSEVT_CODE\n");
          UNUSED(read_blob_resp_event);
          /* USER CODE BEGIN ACI_ATT_READ_BLOB_RESP_VSEVT_CODE */

          /* USER CODE END ACI_ATT_READ_BLOB_RESP_VSEVT_CODE */
        ...
        case ACI_ATT_READ_RESP_VSEVT_CODE:
        {
          ...
          read_resp_event = (aci_att_read_resp_event_rp0 *)p_blecore_evt->data;
          uint8_t i;
          LOG_INFO_APP("  ACI_ATT_READ_RESP_VSEVT_CODE\n");
        ...
        {
          aci_gatt_error_resp_event_rp0 *error_resp_event;
          error_resp_event = (aci_gatt_error_resp_event_rp0 *)p_blecore_evt->data;          
          UNUSED(error_resp_event);
          /* USER CODE BEGIN ACI_GATT_ERROR_RESP_VSEVT_CODE */
          LOG_INFO_APP("  error_resp_event->Error_Code %x\n",error_resp_event->Error_Code);
          if ((error_resp_event->Error_Code == 0x05) && (error_resp_event->Attribute_Handle == (a_ClientContext[0].EAD_Key_MaterialCharHdl)+1)) //insufficient authentication
          {
            LOG_INFO_APP("  ACI_GATT_ERROR_RESP_VSEVT_CODE\n");
            /* pairing req */
            UTIL_SEQ_SetTask(1 << CFG_TASK_SECURITY_REQ_ID, CFG_SEQ_PRIO_0);
          }
          /* USER CODE END ACI_GATT_ERROR_RESP_VSEVT_CODE */
        }
        break;
        default:
          break;
...
/* USER CODE BEGIN gatt_parse_chars_1 */
        ...
        {
          a_ClientContext[index].EAD_Key_MaterialCharHdl = CharStartHdl;
          a_ClientContext[index].EAD_Key_MaterialValueHdl = CharValueHdl;
          LOG_INFO_APP(", ENCRYPTED_DATA_KEY_MATERIAL_UUID charac found\n");
        }
...
/* USER CODE BEGIN gatt_parse_descs_2 */
        ...
        {
          LOG_INFO_APP(", found ENCRYPTED_DATA_KEY_MATERIAL_UUID\n");
        }        
        else if (uuid == ST_P2P_WRITE_CHAR_UUID)
        {
