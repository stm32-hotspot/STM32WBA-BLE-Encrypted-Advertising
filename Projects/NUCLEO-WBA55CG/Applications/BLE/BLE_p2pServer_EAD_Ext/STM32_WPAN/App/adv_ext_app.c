...
//extern uint8_t ead_AdvData[30];
extern uint8_t ead_AdvData[171];
extern uint8_t ead_data_encrypted[180];
extern uint8_t ead_data_no_conn_no_scan_encrypted[250];
extern uint16_t ead_data_encrypted_length;
extern uint16_t ead_data_encrypted_length_NCS;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
static uint8_t ADV_EXT_Build_data_NoConn_NoScan(Adv_Set_Param_t *adv_param);
/* USER CODE END PFP */
...
/* Extended CONNECTABLE with p2pClient_Ext device plus EAD */
  data_cpt = 0;

  data_cpt += sizeof(a_AdvData);
  data_cpt += sizeof(ead_data_encrypted);
  i = 2;
  adv_set_param[i].enable = 1;
  adv_set_param[i].data = NULL;
  adv_set_param[i].data_len = data_cpt;
  strcpy(adv_set_param[i].username, "EADS");
  adv_set_param[i].sid = i;
  adv_set_param[i].interval_min = 200;
  adv_set_param[i].interval_max = 300;
  adv_set_param[i].tx_power = CFG_TX_POWER;
  ...
  adv_set_param[i].address_type = GAP_PUBLIC_ADDR; //GAP_STATIC_RANDOM_ADDR;
  adv_set_param[i].peer_address_type = GAP_PUBLIC_ADDR;
  adv_set_param[i].p_peer_address = &a_peeraddr[0];
  adv_set_param[i].adv_set.Advertising_Handle = i;
  adv_set_param[i].adv_set.Duration = 0;
...
uint8_t ADV_EXT_Config_NoConnectable_NoScannable(void)
{
  uint16_t data_cpt;
  uint8_t i; 
...
/* Extended NO SCANNABLE-NO CONNECTABLE */
  ...
  data_cpt += sizeof(a_AdvData);
  data_cpt += sizeof(ead_data_no_conn_no_scan_encrypted);
  i = 1;
  adv_set_param[i].enable = 1;
  adv_set_param[i].data = NULL;
  adv_set_param[i].data_len = 250;
  strcpy(adv_set_param[i].username, "EADS");
  adv_set_param[i].sid = i;
  adv_set_param[i].interval_min = 200;
  adv_set_param[i].interval_max = 300;
...
/* USER CODE BEGIN FD */
uint8_t ADV_EXT_Start_NoConnectable_NoScannable(void)
{
  tBleStatus status = BLE_STATUS_INVALID_PARAMS;
  uint16_t adv_length_max;  
  uint8_t adv_set_count_supported, loop, adv_set_count;
  ...
  for (loop = 0 ; loop < ADV_SET_COUNT ; loop++)
  {
    if (adv_set_param[loop].enable != 0)
    {
      ...
      if (adv_set_param[loop].data == NULL)
      {
        ...
        status = ADV_EXT_Build_data_NoConn_NoScan(&adv_set_param[loop]);
        if (status == BLE_STATUS_SUCCESS)
        {
          LOG_INFO_APP("  Success: ADV_EXT_Build_data_NoConn_NoScan\n");
        }
        else
        {
          LOG_INFO_APP("  Fail   : ADV_EXT_Build_data_NoConn_NoScan, result: 0x%02X\n", status);
        }
      }
...
uint8_t ADV_EXT_Stop(uint8_t sid)
{
  tBleStatus status = BLE_STATUS_INVALID_PARAMS;  
          
  status = aci_gap_adv_set_enable(0, 1, &adv_set_param[sid].adv_set);
  if (status == BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Success: aci_gap_adv_set_enable, disable\n");
  }
  else
  {
    LOG_INFO_APP("  Fail   : aci_gap_adv_set_enable, disable result: 0x%02X\n", status);
  }
  
  return status;
}

void restart_adv(void)
{
  ADV_EXT_Config();
  ADV_EXT_Start();
}

void restart_adv_noconn_noscan(void)
{
  ADV_EXT_Stop(2);
  ADV_EXT_Config_NoConnectable_NoScannable();
  ADV_EXT_Start_NoConnectable_NoScannable();
//  ADV_EXT_Stop(1);
}
/* USER CODE END FD */
...
static uint8_t ADV_EXT_Build_data(Adv_Set_Param_t *adv_param)
{
  ...
  adv_param->data[data_cpt++] = ead_data_encrypted_length + 1; //length
  adv_param->data[data_cpt++] = AD_TYPE_ENCRYPTED_ADV_DATA;
  for (i=0;i<ead_data_encrypted_length;i++)
  {
    adv_param->data[data_cpt++] = ead_data_encrypted[i];
  }
  ...
  else
  {
    while (manuf_cpt > 0)
    {
      ...
      adv_param->data[data_cpt++] = 0x30; /* ST_MANUF_ID */
      adv_param->data[data_cpt++] = 0x00;
      adv_param->data[data_cpt++] = BLUESTSDK_V2;
      adv_param->data[data_cpt++] = BOARD_ID_NUCLEO_WBA;
      adv_param->data[data_cpt++] = FW_ID_P2P_EADS;
      adv_param->data[data_cpt++] = 0x00;
      adv_param->data[data_cpt++] = 0x00;
      adv_param->data[data_cpt++] = 0x00;
      adv_param->data[data_cpt++] = 0x00;
...
//  adv_param->data[data_cpt++] = ead_data_encrypted_length_NCS + 1; //length
//  adv_param->data[data_cpt++] = AD_TYPE_ENCRYPTED_ADV_DATA;
//  for (i=0;i<ead_data_encrypted_length;i++)
//  {
//    adv_param->data[data_cpt++] = ead_data_no_conn_no_scan_encrypted[i];
//  }
  ...
  manuf_cpt = (adv_param->data_len - data_cpt) - 224;
  LOG_INFO_APP("  manuf_cpt: 0x%02X\n", manuf_cpt);
  LOG_INFO_APP("  data_cpt: 0x%02X\n", data_cpt);
  if(manuf_cpt <= 2)
  {
    status = 1;
  }
  ...
  adv_param->data[data_cpt++] = ead_data_encrypted_length_NCS + 1; //length
  adv_param->data[data_cpt++] = AD_TYPE_ENCRYPTED_ADV_DATA;
  for (i=0;i<ead_data_encrypted_length_NCS;i++)
  {
    adv_param->data[data_cpt++] = ead_data_no_conn_no_scan_encrypted[i];
  }
  return status;
